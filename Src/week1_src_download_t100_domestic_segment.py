...
Requests All Fields, All U.S.. Geographies, And All
Months ForEach Requested Year, And Saves The Untouched ZIP Responses.

Example (Run From The Project Root):
    python src/week1_src_download_t100_domestic_segment.py 
    --start-year 2015 --end-year 2026 --out data/raw
...
def download_year(year: int, out_dir: Path, timeout: int = 600) -> tuple[Path, int, str]:
    ...
    path = out_dir / f"week1_data_t100_domestic_segment_all_carriers_{year}.zip"
...
def write_manifest(rows: list[dict[str, str]], out_dir: Path) -> Path:
    manifest = out_dir / "week1_data_t100_download_manifest.csv"
